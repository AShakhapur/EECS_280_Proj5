...
private:
    ...
    private:
        Key_compare key;
    public:
        bool operator() (const Pair_type &val1, const Pair_type &val2) const {
            return key(val1.first, val2.first);
        }
    };
...
public:
    ...
    Iterator find(const Key_type& k) const {
        pair<Key_type, Value_type> pairs;

        pairs.first = k;
        pairs.second = Value_type();

        return tree.find(pairs);
    }
    ...
    Value_type& operator[](const Key_type& k) {
        if (find(k) == Iterator()) {
            std::pair<Iterator, bool> pairs;
            pairs = insert({ k, Value_type() });
        }
    ...
    std::pair<Iterator, bool> insert(const Pair_type& val) {
        ...
        if (!(find(val.first) == Iterator())) {
            pairs.first = find(val.first);
            pairs.second = false;
            return pairs;
        }

        else {
            pairs.first = tree.insert(val);
            pairs.second = true;
            return pairs;
        }
    }
