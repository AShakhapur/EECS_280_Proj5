...
private:
    ...
    static int size_impl(const Node* node) {
        ...
        if (!node) {
            ...
            size = size_impl(node->left) + size_impl(node->right);
        }
        return size + 1;
    }
    ...
    static Node* copy_nodes_impl(Node* node) {
        if (!node) {
            ...
            Node* node1 = new Node(node->datum, copy_nodes_impl(node->left),
    ...
    static Node* find_impl(Node* node, const T& query, Compare less) {
        if (!node) {
            ...
            if (!less(node->datum, query) && !less(query, node->datum)) {
                // means node->datum and query are equivalent
                return node;
            }
            else {
    ...
    static Node* min_greater_than_impl(Node* node, const T& val, Compare less) {
        ...
        else if (!min_greater_than_impl(node->left, val, less)) {
            return node;
        }
